...
Write-Host "Configuracao de Chaves SSH nas VPS" -ForegroundColor Green
Write-Host ""
...
foreach ($vps in $VPS_LIST) {
    ...
    Write-Host "1. Conecte-se a VPS:" -ForegroundColor Yellow
    ...
    Write-Host "3. Adicione a chave publica:" -ForegroundColor Yellow
    Write-Host "   echo `"$PUBLIC_KEY`" >> ~/.ssh/authorized_keys" -ForegroundColor White
    Write-Host ""
    
    Write-Host "4. Configure permissoes:" -ForegroundColor Yellow
    ...
    Write-Host "6. Teste a conexao (do seu PC):" -ForegroundColor Yellow
...
Write-Host "Apos configurar todas as VPS, voce podera:" -ForegroundColor Green
...
Write-Host "   - Ter autenticacao mais segura" -ForegroundColor White
